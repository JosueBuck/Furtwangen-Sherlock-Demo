...
const showAvailableCollections = async () => {
  ...
  const collectionsContainer = document.getElementById(
    "available-collections-container"
  );
  ...
  for (let i = 0; i < availableCollectionsJson.length; i++) {
    ...
    for (let x = 0; x < availableCollectionsJson[i].systems.length; x++) {
      ...
      for (
        let y = 0;
        y < availableCollectionsJson[i].systems[x].types.length;
        y++
      ) {
...
const getAvailableCollections = async () => {
  try {
    const response = await fetch(
      "http://localhost:3000/getAvailableCollections",
      {
        method: "GET",
      }
    );
...
const showCollectionSchema = async () => {
  ...
  handleContentButtons(
    showSchema,
    setShowSchema,
    "show-collection-schema-button",
    "hide-collection-schema-button"
  );

  const collectionSchemaContainer = document.getElementById(
    "collection-schema-container"
  );
  ...
  const schemaPropertiesNames = Object.getOwnPropertyNames(
...
const getCollectionSchema = async (_collectionName) => {
  try {
    const response = await fetch(
      `http://localhost:3000/getCollectionSchema/${_collectionName}`,
      {
        method: "GET",
      }
    );
...
/* Button Handling */
const handleContentButtons = (
  _currentShowValue,
  _changeShowValueFunction,
  _showButtonId,
  _hideButtonId
) => {
