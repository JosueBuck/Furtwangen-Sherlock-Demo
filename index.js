const API_KEY = "anM6Yjc5YmRkNTYtMTA1Ni00Y2RjLWI1NTEtNTcxZjdmYzU0MmFi";
...
const showAvailableCollections = async () => {
    ...
    const availableCollectionsJson = await getAvailableCollections(API_KEY);
    console.log(availableCollectionsJson);
    ...
    for (let i = 0; i < availableCollectionsJson.length; i++) {
        ...
        // Add Collection to Collection Container
...
const getAvailableCollections = async (_apiKey) => {
    try {
        const response = await fetch('https://furtwangen.sherlock-cloud.de/def-api/search/available-collections', {
            method: 'GET',
            headers: {
                'X-Sherlock-Api-Key': _apiKey
            }
        });
...
const showCollectionSchema = async() => {
    ...
    const collectionSchema = await getCollectionSchema(collectionName, API_KEY);
...
const getCollectionSchema = async (_collectionName, _apiKey) => {
    try {
        const response = await fetch(`https://furtwangen.sherlock-cloud.de/def-api/search/${_collectionName}/schema`, {
            method: 'GET',
            headers: {
                'X-Sherlock-Api-Key': _apiKey
            }
